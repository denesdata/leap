...
var minTextRadius = 10; //diameter of smallest bubble to receive a label
var minFontSize = 8; //font size of smallest bubble to receive a label
    ...
    function filterTable() {
        ...
        d3.selectAll('.tableEntry')
            .style('display', function(e) {
                ...
                onNodes.forEach(function(d) {
                    if (e.keywords.indexOf(d.id) > -1) {
                        display = 'table-row'
                        matches.push(d.id);
                    }
                })
                d3.select(this).select('.matches').text(matches)
                return display
            })
...
// Load the data.
d3.json('data/nodes_edges_table.json', display);
