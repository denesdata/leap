...
function bubbleChart() {
    ...
    function createTable() {
        ...
        stakeholderTable.append('td')
            ...
            .on('click', function(d) {
                d3.select(this).classed('stakeholder-hidden', (d3.select(this).classed('stakeholder-hidden') ? false : true));
                d3.select(this).select('.stakeholder-icon').text(d3.select(this).classed('stakeholder-hidden') ? '◢' : '◤');
            })
    }
...
// Load the data.
d3.json('data/nodes_edges_table_lessons.json', display);
