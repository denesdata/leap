...
function bubbleChart() {
    ...
    function createTable() {
        ...
        stakeholderTable.append('td')
            ...
            .html(function(d) {
                ...
                return '<div class="stakeholder-title"><span class="stakeholder-icon">◢</span>&nbsp;&nbsp;<span>' + d.name + '</span></div>' +
                    ...
                    '<b>Webpage</b>: <a href="' + d.web + '" target="_blank">' + d.web + ' </a><br>' +
                    '<b>Social media</b>: <a href="https://' + d.fb + '" target="_blank">' + d.fb + ' </a><br>' +
        ...
        d3.selectAll('#stakeholder-head')
            .on('click', function(d) {
                d3.select('#stakeholder-table-parent').classed('stakeholder-head-hidden', (d3.select('#stakeholder-table-parent').classed('stakeholder-head-hidden') ? false : true));
                d3.select('.stakeholder-head-icon').text(d3.select('#stakeholder-table-parent').classed('stakeholder-head-hidden') ? '◢' : '◤');
            })
    }
    ...
    function createLessons() {
        ...
        var lessonsTable = d3.select('#lessons-table')
            ...
            .style('display', 'none')
            .style('border-color', function(d) { return d3.rgb(fillColor(d.Priority)) })
            .style('background-color', function(d) {
                var c = d3.rgb(fillColor(d.Priority));
                c.opacity = 0.2;
                return c;
            })
        ...
        lessonsTable.append('div')
            ...
            .attr('src', function(d) { return 'img/type/' + replaceSpaces(d.Priority) + '_' + typ[d.Type] + '.png' })
        lessonsTable.append('div')
            .classed('matches', true)
        lessonsTable.append('div')
            .classed('lesson', true)
            .classed('lesson-hidden', true)
            .html(function(d) {
                return '<div class="lesson-body">' +
                    // '<tr><td><b>Priority</b></td><td>' + d.Priority + '</td></tr>' +
                    // '<tr><td><b>Type</b></td><td>' + d.Type + '</td></tr>' +
                    ((d.Type == 'Policy recommendations') ? '<div class="pol">★ Proposed action</div><div class="lesson">' + d.Quote + '</div>' : '') +
                    ...
                    ((d.Type == 'Qualitative research') ? '<div class="lesson">' + d.Sentence + '</div>' : '') +
                    ...
                    ((d.Type == 'Qualitative research') ? '<div class="quote"><table><tr><td>&ldquo;</td><td>' + d.Quote +
                        '</td></tr></table></div>' : '') +
                    ((d.Type == 'Quantitative research') ? '<div class="lesson">' + d.Quote + '</div>' : '') +
                    ((d.Type == 'Quantitative research') ? '<div><img src="src/8.png" style="width:180px;"></div>' : '') +
                    '</div>'
            })
        d3.selectAll('#lessons-head')
            .on('click', function(d) {
                d3.select('#lessons-table-parent').classed('lessons-head-hidden', (d3.select('#lessons-table-parent').classed('lessons-head-hidden') ? false : true));
                d3.select('.lessons-head-icon').text(d3.select('#lessons-table-parent').classed('lessons-head-hidden') ? '◢' : '◤');
            })
    }
    ...
    function filterLessons() {
        ...
        d3.selectAll('.lessonEntry')
            .style('display', function(lesson) {
                var display = 'none'
                var matches = new Array();
                var matchText = ''
                onNodes.forEach(function(node) {
                    if (Object.keys(lesson.keywords).indexOf(node.id) > -1) {
                        // if (lesson.Keyword == node.id) {
                        display = 'table-row'
                        matches.push(node.id);
                        matchText = matchText + '<span class="keyword" style="background-color:' +
                            d3.rgb(fillColor(node.priority)) + ';">' + node.id.replace(' ', '&nbsp;') + '</span> '
